...
# ฟังก์ชันแสดงข้อความผู้ชนะ
def display_victory(gameDisplay, display_width, display_height, winner_img, winner_text):
    gameDisplay.blit(winner_img, (display_width / 2 - 50, display_height / 2 - 100))
    font = pygame.font.Font('freesansbold.ttf', 50)
    text_surface, text_rect = displayText.display_text(winner_text, font)
    text_rect.center = (display_width / 2, display_height / 2 + 50)
    gameDisplay.blit(text_surface, text_rect)
    
    # ตัวแปรสำหรับการแสดงท่าดีใจ
goal_scored = False
goal_time = 0
goal_display_duration = 1.5  # ระยะเวลาการแสดงท่าดีใจ 1.5 วินาที
score_left = 0
score_right = 0

# ฟังก์ชันตรวจสอบการทำประตูและแสดงท่าดีใจ
def check_goal(ball_x, ball_y):
    global score_left, score_right, goal_scored, goal_time
    if ball_x < goal_width and ball_y > display_height - goal_height:
        score_left += 1
        goal_scored = True
        goal_time = time.time()
    elif ball_x > display_width - goal_width and ball_y > display_height - goal_height:
        score_right += 1
        goal_scored = True
        goal_time = time.time()
...
crashed = False
ball_x, ball_y = 600, 300  # ตำแหน่งเริ่มต้นของลูกบอล
while not crashed:
    ...
    game_background()
    remaining_time = display_timer(gameDisplay, display_width, display_height)
    ...
    # ลบภาพผู้เล่นเก่าออกโดยการเติมพื้นหลังในบริเวณที่ผู้เล่นอยู่
        game_background()  
    # ตรวจสอบและแสดงท่าดีใจเมื่อทำประตูได้
    if goal_scored and time.time() - goal_time < goal_display_duration:
        if score_left > score_right:
            screen.blit(boyFirstLeftVictoryImg, (display_width / 2 - 50, display_height / 2 - 50))
        else:
            screen.blit(boySecondRightVictoryImg, (display_width / 2 - 50, display_height / 2 - 50))
    else:
        goal_scored = False  # รีเซ็ตสถานะการทำประตู
    ...
    if remaining_time <= 0:
        if score_left > score_right:
            display_victory(screen, display_width, display_height, boyFirstLeftVictoryImg, f"Player 1 ชนะด้วยสกอร์ {score_left}!")
        elif score_right > score_left:
            display_victory(screen, display_width, display_height, boySecondRightVictoryImg, f"Player 2 ชนะด้วยสกอร์ {score_right}!")
        else:
            display_victory(screen, display_width, display_height, None, "เสมอ!")
        pygame.display.update()
        pygame.time.delay(3000)
        crashed = True  # ออกจากลูปเกม
