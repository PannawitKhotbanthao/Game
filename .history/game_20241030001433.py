...
# ฟังก์ชันที่เรียกเมื่อเวลาหมด
def check_end_game(score_left, score_right):
    if score_left > score_right:
        winner_text = f"Player 1 ชนะด้วยสกอร์ {score_left}!"
        display_victory(gameDisplay, display_width, display_height, boyFirstLeftVictoryImg, winner_text)
    elif score_right > score_left:
        winner_text = f"Player 2 ชนะด้วยสกอร์ {score_right}!"
        display_victory(gameDisplay, display_width, display_height, boySecondRightVictoryImg, winner_text)
    else:
        winner_text = "เสมอ!"
...
while not crashed:
    ...
    if remaining_time <= 0:
        # เรียกใช้ check_end_game เพื่อประกาศผู้ชนะและแสดงผลลัพธ์
        check_end_game(score_left, score_right)
        
        # แสดงปุ่มให้เลือกรีเซ็ตเกมหรือออกเกม
