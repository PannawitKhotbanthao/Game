...
def game_background():
    ...
    screen.blit(backImg_frames[background_frame_index], (0, 0))
    # อัปเดต background_frame_index เฉพาะสำหรับพื้นหลัง
    background_frame_index = (background_frame_index + 1) % len(backImg_frames)
...
# ตัวแปร frame_index สำหรับแอนิเมชันพื้นหลังเท่านั้น
frame_index = 0

def game_background():
    global frame_index
    # แสดงภาพเฟรมปัจจุบันจาก backImg_frames
    screen.blit(backImg[frame_index], (0, 0))
    # อัปเดต frame_index เฉพาะสำหรับพื้นหลังเท่านั้น
    frame_index = (frame_index + 1) % len(backImg)
