...
goalLeftImg = pygame.image.load('images/goalLeft1.png')
goalRightImg = pygame.image.load('images/goalRight1.png')   
...
def display_buttons(gameDisplay, display_width, display_height):
    reset_game = buttons.button(gameDisplay, "reset", display_width / 3, display_height / 2, 100, 50, (0, 255, 0), (0, 200, 0), 'reset')
    exit_game = buttons.button(gameDisplay, "exit", display_width * 2 / 3, display_height / 2, 100, 50, (255, 0, 0), (200, 0, 0), 'exit')
    
    # ตรวจสอบการกดปุ่ม
    if reset_game == 'reset':
        # เรียกหน้าเริ่มต้น
        intro.game_intro(gameDisplay, display_width, display_height, sound)
        # รีเซ็ตสกอร์และเวลาที่เริ่มนับใหม่
        global score_left, score_right, start_time  # ใช้ global สำหรับตัวแปรที่ใช้ข้างนอก
        score_left = 0
        score_right = 0
    ...
    # คืนค่า reset_game, exit_game
    return reset_game, exit_game


def button(gameDisplay, msg, x, y, w, h, ic, ac, action=None, font=None):
    mouse = pygame.mouse.get_pos()
    click = pygame.mouse.get_pressed()

    # ตั้งค่าฟอนต์เริ่มต้นหากไม่ได้กำหนดฟอนต์ใหม่
    # if font is None:
    #     font = pygame.font.Font("freesansbold.ttf", 20)

    # การแสดงปุ่มพร้อมฟอนต์ที่เลือก
    ...
    textSurf, textRect = displayText.display_text(msg, font)
...
while not crashed:
    ...
    pygame.display.update()  # อัปเดตหน้าจอเพื่อให้แสดงผลได้ครบถ้วน

    # ลูปที่รอให้ผู้เล่นกดปุ่ม reset หรือ exit
    waiting_for_input = True
    while waiting_for_input:
        # เคลียร์พื้นหลังและแสดงผู้ชนะ
        game_background()
        if score_left > score_right:
            gameDisplay.blit(boyFirstLeftVictoryImg, (display_width / 2 - 50, display_height / 2 + 100))
        elif score_right > score_left:
        ...
        # แสดงปุ่ม
        reset_game, exit_game = display_buttons(gameDisplay, display_width, display_height)
        ...
        if reset_game == 'reset':
            ...
            score_left = 0
            score_right = 0
            start_time = time.time()  # เริ่มนับเวลาใหม่
            intro.game_intro(gameDisplay, display_width, display_height, sound)  # เรียกหน้าเริ่มต้น
            waiting_for_input = False  # ออกจากลูป

        elif exit_game == 'exit':
            pygame.quit()
            quit()

        for event in pygame.event.get():  # ตรวจสอบเหตุการณ์
            if event.type == pygame.QUIT:
                pygame.quit()
                quit()

        pygame.display.update()  # อัปเดตหน้าจอในลูปการรอ
        clock.tick(15)  # จำกัดความถี่การอัปเดตหน้าจอ
    ...
    TextSurf, TextRect = displayText.display_text(score_text, font)
