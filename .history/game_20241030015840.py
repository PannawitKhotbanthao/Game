...
if remaining_time <= 0:  # เมื่อหมดเวลา
    if score_left > score_right:
        result = Paused.paused(gameDisplay, display_width, display_height, "Game Over", pause=True, winner="Player 1 ชนะ!")
    elif score_right > score_left:
        result = Paused.paused(gameDisplay, display_width, display_height, "Game Over", pause=True, winner="Player 2 ชนะ!")
    else:
        result = Paused.paused(gameDisplay, display_width, display_height, "Game Over", pause=True, winner="เสมอกัน!")

    if result == "restart":
        # รีเซ็ตค่าคะแนน และอื่น ๆ ที่จำเป็น
        score_left = 0
        score_right = 0
        start_time = time.time()  # รีเซ็ตเวลา
        intro.game_intro(gameDisplay, display_width, display_height, sound)  # กลับไปหน้าเริ่มต้น
