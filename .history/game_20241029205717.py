...
# backImg = pygame.image.load('images/Campnu.gif')
...
# ตัวแปร frame_index สำหรับแอนิเมชันพื้นหลัง
background_frame_index = 0
background_fps = 10  # ควบคุมความเร็วแอนิเมชันของพื้นหลัง

# ตัวแปร frame_index และตัวจับเวลา สำหรับแอนิเมชันผู้เล่น
player_frame_index = 0
player_animation_speed = 200  # มิลลิวินาที (ปรับให้เหมาะสม)
last_player_frame_update = pygame.time.get_ticks()

def game_background():
    global background_frame_index
    # แสดงภาพเฟรมปัจจุบันจาก backImg_frames
    screen.blit(backImg[background_frame_index], (0, 0))
    # อัปเดต background_frame_index เฉพาะสำหรับพื้นหลัง
    background_frame_index = (background_frame_index + 1) % len(backImg)

def animate_player(player_frames, x, y):
    global player_frame_index, last_player_frame_update
    # ตรวจสอบเวลาที่ผ่านไปตั้งแต่เฟรมล่าสุด
    current_time = pygame.time.get_ticks()
    if current_time - last_player_frame_update > player_animation_speed:
        player_frame_index = (player_frame_index + 1) % len(player_frames)
        last_player_frame_update = current_time
    # แสดงเฟรมปัจจุบันของผู้เล่น
    gameDisplay.blit(player_frames[player_frame_index], (x, y))
...
# ตั้งค่าตัวจับเวลาแยกสำหรับพื้นหลังและการอัปเดตเกมทั่วไป
background_timer = pygame.time.get_ticks()
background_update_interval = 1000 // background_fps  # คำนวณความถี่ในการอัปเดตพื้นหลัง
...
while not crashed:
    ...
    # อัปเดตพื้นหลังเมื่อถึงช่วงเวลา
    current_time = pygame.time.get_ticks()
    if current_time - background_timer > background_update_interval:
        game_background()
        background_timer = current_time  # รีเซ็ตตัวจับเวลาใหม่
