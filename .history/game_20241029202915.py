...
# เพิ่มตัวแปร frame_index ไว้นอกฟังก์ชันเพื่อติดตามเฟรมปัจจุบัน
frame_index = 0
...
def game_background():
    ...
    # อัปเดต frame_index เพื่อไปยังเฟรมถัดไป
    frame_index = (frame_index + 1) % len(backImg)  # วนกลับไปที่เฟรมแรกเมื่อถึงเฟรมสุดท้าย
...
# ประกาศตัวแปร frame_index เพื่อควบคุมเฟรมปัจจุบันของพื้นหลัง
frame_index = 0

def background(x, y):
    global frame_index
    # แสดงเฟรมปัจจุบันจาก backImg_frames
    gameDisplay.blit(backImg[frame_index], (x, y))
    # อัปเดต frame_index เพื่อไปยังเฟรมถัดไป
    frame_index = (frame_index + 1) % len(backImg)  # วนกลับไปที่เฟรมแรกเมื่อถึงเฟรมสุดท้าย
...
while not crashed:
    ...
    # คำนวณแรงโน้มถ่วงและการเคลื่อนที่ของลูกบอลในลูปหลัก
    verticalSpeed[0] += gravityAcceleration  # เพิ่มแรงโน้มถ่วงในแนวดิ่ง
    ball_y += verticalSpeed[0]  # อัปเดตตำแหน่งแนวตั้งของลูกบอล
    ball_x += horizontalSpeed[0]  # อัปเดตตำแหน่งแนวนอนของลูกบอล
        # ตรวจสอบการชนกับขอบและพื้น
    if ball_y > (display_height - ball_height):
        ball_y = display_height - ball_height
        verticalSpeed[0] = -verticalSpeed[0] * 0.7  # เด้งกลับพร้อมลดความเร็ว
        horizontalSpeed[0] *= 0.95  # ลดความเร็วแนวราบเมื่อชนพื้น
    
    # เรียกแสดงลูกบอลที่ตำแหน่งใหม่
    ball(ball_x, ball_y)
