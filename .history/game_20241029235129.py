...
while not crashed:
    ...
    if ball_x > (display_width - goal_width) and ball_x < display_width and ball_y < display_height and ball_y > display_height - goal_height:score_right += 1

    # แสดงภาพท่าดีใจสำหรับ Player 2 ที่ตำแหน่งกลางจอ
    gameDisplay.blit(backImg, (0, 0))  # เติมพื้นหลังใหม่
    gameDisplay.blit(boySecondRightVictoryImg, (display_width / 2 - 50, display_height / 2 - 50))
    pygame.display.update()
    pygame.time.delay(1000)  # หน่วงเวลาเพื่อแสดงท่าดีใจ
    
    # ตั้งค่าลูกบอลกลับไปที่ตำแหน่งเริ่มต้น
    ball_x = 600
    ball_y = 300 - (ball_width / 2)
    gravityAcceleration = 0.3
    horizontalAcceleration = 0
